...
export function CollapsiblePanel({
  ...
  if (minimizableToChip && chipped) {
    return (
      <button
        ...
        className="pointer-events-auto flex max-w-[200px] items-center gap-2 self-end rounded-md border border-ink-800 bg-ink-950/95 px-2.5 py-1.5 text-left shadow-[0_4px_14px_rgba(0,0,0,0.4)] backdrop-blur transition hover:bg-ink-900/90"
        title={`Expand ${title}`}
      >
  ...
  return (
    <section className="pointer-events-auto w-full max-w-[360px] self-end rounded-md border border-ink-800 bg-ink-950/90 backdrop-blur shadow-[0_4px_14px_rgba(0,0,0,0.4)]">
      <div className="flex w-full items-center gap-0">
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="flex min-w-0 flex-1 items-center gap-2 px-3 py-2 text-left transition hover:bg-ink-900/60"
        >
          {open ? (
        ...
        {minimizableToChip && (
          <button
            ...
            className="shrink-0 border-l border-ink-800 px-2.5 py-2 text-slate2-400 transition hover:bg-ink-900/60 hover:text-slate2-200"
      ...
      {open && (
        <div className="max-h-[min(46vh,420px)] overflow-y-auto border-t border-ink-800 px-3 py-2">
          {children}
        </div>
      )}
    </section>
