...
export function Sidebar({ ships, bySource, selected, isLoading, errors, onSelect }: Props) {
    ...
    <div className="flex h-full flex-col">
      <header className="border-b border-ink-800 px-4 py-3">
        ...
        <p className="mt-0.5 text-xs text-slate2-400">
          Mapbox + FastAPI {isLoading ? "loading..." : `${ships.length.toLocaleString()} vessels`}
        </p>
      </header>

      <section className="border-b border-ink-800 px-4 py-3 space-y-3">
            ...
            return (
              <button
                ...
                className={`flex w-full items-center justify-between rounded px-2 py-1.5 text-left text-xs transition ${
                  on ? "bg-ink-800 text-slate2-200" : "bg-transparent text-slate2-400 hover:bg-ink-900"
                }`}
              >
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            className="flex-1 rounded border border-ink-800 bg-ink-900 px-2 py-1 text-xs text-slate2-200 focus:border-accent-safe focus:outline-none"
          >
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded border border-ink-800 bg-ink-900 py-1.5 pl-7 pr-2 text-xs text-slate2-200 placeholder:text-slate2-400 focus:border-accent-safe focus:outline-none"
          />
        </div>
      </section>
            ...
            <li key={`${s.source}:${s.mmsi}`}>
              <button
                ...
                className={`flex w-full items-center justify-between gap-2 rounded px-2 py-1.5 text-left transition ${
                  isSel ? "bg-ink-700 text-slate2-200" : "hover:bg-ink-900 text-slate2-400"
                }`}
              >
      ...
      {errors.length > 0 && (
        <div className="border-t border-ink-800 bg-ink-900 px-4 py-2 text-[11px] text-accent-suspect">
          {errors.map((e, i) => (
            <div key={i}>{e}</div>
          ))}
        </div>
