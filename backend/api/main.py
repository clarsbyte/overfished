"""FastAPI surface for the demo REST API (vessels, heatmap, species, Twilio).
...
The Vite dev server proxies ``/api`` here on port **8000**. LangChain agents,
sequence training, and ``POST /comms/call`` live on the separate BFF in
``api/`` (typically **8001** — see ``frontend/vite.config.ts``).
